...
UI: 期間選択（「1日」「1週間」「1ヶ月」のラジオボタンまたはプルダウン）と、箱数選択（「1/2箱」～「10箱」のスライダーまたはプルダウン）。
...
UI: デフォルトの参加費を表示し、ユーザーが100円単位で参加費を調整できる入力欄（スライダーまたは直接入力）。
...
オプション1: 「返金を受け取る」
- 説明: 記録日数に応じて、参加費の一部が返金されます
- 手数料: 返金処理手数料として500円がかかります
- 計算方法: （参加費 - 500円）× 記録日数 ÷ 30日
- 例: 参加費10,000円、20日記録した場合 → （10,000円 - 500円）× 20 ÷ 30 = 6,333円が返金

オプション2: 「募金する」
- 説明: 記録日数に応じた金額を、選択した団体に寄付します
- 手数料: なし（参加費の全額が対象）
- 計算方法: 参加費 × 記録日数 ÷ 30日
- 例: 参加費10,000円、20日記録した場合 → 10,000円 × 20 ÷ 30 = 6,667円を募金

目標金額の表示:
- 返金選択時: 「目標取り戻し金額: 〇〇円（参加費 - 500円）」
- 募金選択時: 「目標募金額: 〇〇円（参加費）」
...
重要なルール:
- 禁煙に成功した日も、失敗した日も、記録をつけることで「記録成功日」としてカウントされます
- 記録をつけなかった日は、カウントされません
- 30日間毎日記録をつけた場合、返金選択なら満額（参加費-500円）、募金選択なら満額（参加費）となります
...
処理: 記録成功日として記録。マネーモンスターに「ダメージ」を与える演出と、取り戻し予定額の増加。
...
進捗画面でマネーモンスターのビジュアルを表示し、取り戻した金額に応じてモンスターの姿や、モンスターが持つ財産の量が変化する演出。

3.5. 通知機能
ユーザーが設定した記録時間になったら、記録を促すプッシュ通知。

3.6. 募金証明機能 (Donation Proof Feature)
※募金を選択したユーザーのみ対象
...
オンボーディング時に、Stripe CheckoutまたはElementsを利用してユーザーから設定された参加費を徴収する。

30日間のチャレンジ開始時に一括で徴収。

返金・募金実施:

チャレンジ終了時（30日後）、またはユーザーが30日以内にチャレンジを中断した場合、その時点での記録成功日数に応じて以下を実施:

返金選択ユーザー: 
  - 返金額の計算式: (参加費 - 500円) × (記録成功日 / 30)
  - Stripe経由で計算された金額を返金

募金選択ユーザー:
  - 募金額の計算式: 参加費 × (記録成功日 / 30)
  - アプリ運営側が集計した金額を定期的に各募金先に寄付

収益の計上:

返金選択ユーザーからの収益: 
  - 手数料500円 + 参加費から返金されなかった残りの部分
  - 計算式: 500円 + (参加費 - 500円) × (1 - 記録成功日 / 30)
