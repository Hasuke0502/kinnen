...
UI: デフォルトの参加費を表示し、ユーザーが0円から任意の金額まで100円単位で参加費を調整できるプルダウン。0円での参加も可能。
...
クイックアクション機能は実装しない。ユーザーは各機能へのナビゲーションリンクを通じて必要な画面にアクセスする。
...
3.6. 音響効果機能 (Sound Effects Feature)
ゲーム体験を向上させ、ユーザーのモチベーションを高めるための音響効果機能を実装する。

音の種類と使用場面:

ボタンクリック音:
- 全てのボタン（記録ボタン、設定ボタン、ナビゲーションボタンなど）をクリックした際に再生
- 軽やかで心地よいクリック音を使用

記録成功音:
- 毎日の記録を完了した際に再生される達成感のある音
- 禁煙成功時は特に爽快感のある音

マネーモンスターダメージ音:
- マネーモンスターにダメージを与える際の効果音
- ゲーム感を演出する攻撃音やダメージ音

進捗アップデート音:
- 進捗が更新される際の通知音
- お金を取り戻す音として、コイン音やレジスター音

設定とユーザビリティ:
- 音響効果のON/OFF切り替え機能を設定画面に追加
- 初期設定ではONに設定
- 音量調整機能（小・中・大の3段階）
- 公共の場での使用を考慮し、デバイスのサイレントモードを尊重

技術実装:
- HTML5 Audio要素とWeb Audio APIを併用
- MP3音声ファイルの使用（軽量化された高品質なサウンド）
- プリロード機能による遅延の最小化
- フォールバック機能（音声ファイル読み込み失敗時の代替音）
- 音声ファイル配置場所：/public/sounds/フォルダ

3.7. 募金証明機能 (Donation Proof Feature)
※募金を選択したユーザーのみ対象
